...
const CatalogView = ({ products, categories, loading, error, filters, onFilterChange }) => {
  ...
  const filteredProducts = useMemo(() => {
    const result = products.filter(product => {
      ...
      if (filters.selectedCategories.length > 0) {
        // Safely extract category ID(s)
        let productCategoryIds = [];

        if (product.categorias) {
          // Case 1: categorias.data is an array
          if (Array.isArray(product.categorias?.data)) {
            productCategoryIds = product.categorias.data.map(cat => cat.id || cat);
          }
          // Case 2: categorias is an array
          else if (Array.isArray(product.categorias)) {
            productCategoryIds = product.categorias.map(cat => cat.id || cat);
          }
          // Case 3: categorias is a single object
          else if (typeof product.categorias === 'object' && product.categorias.id) {
            productCategoryIds = [product.categorias.id];
          }
        }
      ...
      console.log(`${product.Nombre}: $${price} in range [$${filters.priceRange.min}-$${filters.priceRange.max}]? ${inRange}`);
    ...
    console.log(`==> Filtered: ${result.length}/${products.length} products with range $${filters.priceRange.min}-$${filters.priceRange.max}`);
    return result;
  }, [products, filters]);
            ...
            {loading ? (
              ...
              <>
                ...
                >
                  ...
                  <CategoryDescription>
                    Todos nuestros productos frescos y de calidad en un solo lugar.
                  </CategoryDescription>
                </HeroSection>
...
// --- Styled Components (copied from Catalog.jsx) ---
